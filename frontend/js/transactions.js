  ...
  try {
    ...
    result.items.forEach((transaction) => {
      const txDate = new Date(
        /(?:Z|[+-]\d{2}:\d{2})$/.test(transaction.createdAt)
    ...
    <span class="transaction-amount">${transaction.type === "Deposit" || isIncoming ? "+" : "-"}$${transaction.amount.toLocaleString(
      "en-CA",
      {
