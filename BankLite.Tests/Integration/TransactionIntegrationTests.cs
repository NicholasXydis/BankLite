...
public class TransactionIntegrationTests : IAsyncLifetime
{
    ...
    private async Task<(string Token, Guid AccountId)> RegisterAndCreateAccountAsync(
    ...
    public async Task GetTransactionsByDateRange_Unauthenticated_Returns401()
    {
        ...
        var end = DateTime.UtcNow.AddDays(1).ToString("o");
        var response =
            await _client.GetAsync($"/api/transaction/{Guid.NewGuid()}/range?startDate={start}&endDate={end}");
