...
public class TransactionServiceTests
{
    ...
    public TransactionServiceTests()
    {
        ...
        var mockBalanceNotifier = new Mock<IBalanceNotifier>();
    ...
    private Transaction CreateTransaction(Guid accountId, decimal amount, TransactionType type, string description = "")
    {
    ...
    public async Task DepositAsync_DuplicateIdempotencyKey_ReturnsExistingTransaction()
    {
        var userId = Guid.NewGuid();
        var account = CreateAccount(userId);
        var existingTransaction = CreateTransaction(account.Id, 250, TransactionType.Deposit, "Deposit of $250.00");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(account.Id)).ReturnsAsync(account);
        _mockTransactionRepo.Setup(r => r.GetByIdempotencyKeyAsync(It.Is<string>(key => key != "test-key")))
        ...
        _mockTransactionRepo.Verify(r => r.GetByIdempotencyKeyAsync("test-key"), Times.Never);
    ...
    public async Task WithdrawAsync_DuplicateIdempotencyKey_ReturnsExistingTransaction()
    {
        ...
        var existingTransaction =
            CreateTransaction(account.Id, 250, TransactionType.Withdrawal, "Withdrawal of $250.00");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(account.Id)).ReturnsAsync(account);
        _mockTransactionRepo.Setup(r => r.GetByIdempotencyKeyAsync(It.Is<string>(key => key != "test-key")))
            .ReturnsAsync(existingTransaction);
        var dto = new DepositWithdrawDto { AccountId = account.Id, Amount = 250 };
    ...
    [Fact]
    public async Task WithdrawAsync_InsufficientFunds_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var account = CreateAccount(userId);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(account.Id)).ReturnsAsync(account);
    ...
    public async Task TransferAsync_ValidRequest_MovesMoneyBetweenAccounts()
    {
        ...
        var toAccount = CreateAccount(userId, 1000, "ACC002");
    ...
    public async Task TransferAsync_ValidRequest_CreatesTwoTransactions()
    {
        ...
        var toAccount = CreateAccount(userId, accountNumber: "ACC002");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(toAccount.Id)).ReturnsAsync(toAccount);
    ...
    [Fact]
    public async Task TransferAsync_InsufficientFunds_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId, 100);
        var toAccount = CreateAccount(userId, accountNumber: "ACC002");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(toAccount.Id)).ReturnsAsync(toAccount);
        var dto = new TransferDto { FromAccountId = fromAccount.Id, ToAccountId = toAccount.Id, Amount = 500 };
    ...
    [Fact]
    public async Task TransferAsync_ToAccountNotFound_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(It.Is<Guid>(id => id != fromAccount.Id)))
    ...
    [Fact]
    public async Task TransferAsync_ToAccountOwnedByDifferentUser_ThrowsUnauthorizedAccessException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId, accountNumber: "ACC001");
    ...
    public async Task TransferExternalAsync_ValidRequest_MovesMoneyBetweenAccounts()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId);
        var toAccount = CreateAccount(Guid.NewGuid(), 500, "ACC002");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
    ...
    public async Task TransferExternalAsync_DuplicateIdempotencyKey_ReturnsEarlyWithoutUpdating()
    {
        ...
        var fromAccount = CreateAccount(userId);
        var toAccount = CreateAccount(Guid.NewGuid(), 500, "ACC002");
        var existingTransaction = CreateTransaction(fromAccount.Id, 300, TransactionType.Transfer,
            $"External Transfer to account {toAccount.Id}");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
        _mockAccountRepo.Setup(r => r.GetByAccountNumberAsync("ACC002")).ReturnsAsync(toAccount);
        _mockTransactionRepo.Setup(r => r.GetByIdempotencyKeyAsync(It.Is<string>(key => key != "test-key")))
            .ReturnsAsync(existingTransaction);
    ...
    [Fact]
    public async Task TransferExternalAsync_ToAccountNotFound_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
    ...
    [Fact]
    public async Task TransferExternalAsync_SameAccount_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId, accountNumber: "ACC001");
        _mockAccountRepo.Setup(r => r.GetByIdAsync(fromAccount.Id)).ReturnsAsync(fromAccount);
    ...
    [Fact]
    public async Task TransferExternalAsync_InsufficientFunds_ThrowsBadRequestException()
    {
        var userId = Guid.NewGuid();
        var fromAccount = CreateAccount(userId, 100);
        var toAccount = CreateAccount(Guid.NewGuid(), accountNumber: "ACC002");
    ...
    public async Task GetTransactionsByAccountIdAsync_ValidRequest_ReturnsPagedResult()
    {
        ...
        var account = CreateAccount(userId);
        _mockAccountRepo.Setup(r => r.GetByIdAsync(account.Id)).ReturnsAsync(account);
        _mockTransactionRepo.Setup(r => r.GetByAccountIdAsync(account.Id, 1, 10, null))
    ...
    public async Task GetTransactionsByAccountIdAsync_UnauthorizedUser_ThrowsUnauthorizedAccessException()
    {
        ...
        var act = async () =>
            await _transactionService.GetTransactionsByAccountIdAsync(account.Id, Guid.NewGuid(), 1, 10);

        await act.Should().ThrowAsync<UnauthorizedAccessException>();
        _mockTransactionRepo.Verify(
            r => r.GetByAccountIdAsync(It.IsAny<Guid>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>()),
            Times.Never);
    }
    ...
    public async Task GetTransactionsByDateRangeAsync_ValidRequest_ReturnsTransactions()
    {
        ...
        _mockTransactionRepo
            .Setup(r => r.GetByAccountIdAndDateRangeAsync(account.Id, It.IsAny<DateTime>(), It.IsAny<DateTime>()))
            .ReturnsAsync(new List<Transaction>());

        var result = await _transactionService.GetTransactionsByDateRangeAsync(account.Id, userId,
    ...
    public async Task GetTransactionsByDateRangeAsync_UnauthorizedUser_ThrowsUnauthorizedAccessException()
    {
        ...
        var act = async () => await _transactionService.GetTransactionsByDateRangeAsync(account.Id, Guid.NewGuid(),
            DateTime.UtcNow.AddDays(-7), DateTime.UtcNow);
