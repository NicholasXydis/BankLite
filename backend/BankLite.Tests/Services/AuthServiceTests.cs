...
public class AuthServiceTests
{
    ...
    public AuthServiceTests()
    {
        ...
        mockUnitOfWork.Setup(u => u.ExecuteInTransactionAsync(It.IsAny<Func<Task>>()))
            .Returns((Func<Task> operation) => operation());
        mockTokenService.Setup(t => t.GenerateAccessToken(It.IsAny<User>())).Returns("access-token");
        mockTokenService.Setup(t => t.GetAccessTokenExpiry()).Returns(DateTime.UtcNow.AddMinutes(60));
    ...
    private User CreateTestUser(string email = "test@banklite.com", string password = "Password123",
        int failedAttempts = 0, DateTime? lockoutEnd = null)
    {
        return new User
        {
            Id = Guid.NewGuid(),
    ...
    [Fact]
    public async Task LoginAsync_AccountLocked_ThrowsBadRequestException()
    {
    ...
    public async Task LoginAsync_WrongPassword_IncrementsFailedAttempts()
    {
        ...
        await Assert.ThrowsAsync<BadRequestException>(() => _authService.LoginAsync(dto));
    ...
    public async Task RegisterAsync_DuplicateEmail_NeverCallsAddAsync()
    {
        ...
        await Assert.ThrowsAsync<BadRequestException>(() => _authService.RegisterAsync(dto));
    ...
    public async Task ForgotPasswordAsync_UserExists_SendsEmail()
    {
        ...
        _mockEmailService.Verify(e => e.SendPasswordResetEmailAsync(user.Email, It.IsAny<string>(), It.IsAny<string>()),
            Times.Once);
    }
    ...
    [Theory]
    [InlineData(null)]
    public async Task ResetPasswordAsync_TokenNotFound_ThrowsBadRequestException(PasswordResetToken? token)
    {
    ...
    [Fact]
    public async Task ResetPasswordAsync_AlreadyUsedToken_ThrowsBadRequestException()
    {
        var user = CreateTestUser();
        var resetToken = new PasswordResetToken
        {
    ...
    public async Task RevokeRefreshTokenAsync_ValidToken_RevokesToken()
    {
        ...
        _mockRefreshTokenRepo.Setup(r => r.GetByTokenAsync(HashToken("valid-refresh-token")))
    ...
    public async Task RevokeRefreshTokenAsync_AlreadyRevokedToken_NeverCallsRevokeAsync()
    {
        ...
        _mockRefreshTokenRepo.Setup(r => r.GetByTokenAsync(HashToken("already-revoked-token")))
            .ReturnsAsync(refreshToken);
