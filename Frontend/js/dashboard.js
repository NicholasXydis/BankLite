async function loadDashboard() {
  ...
  try {
    ...
    accounts.forEach((account) => {
      ...
      card.dataset.testid = "account-card";
    ...
    document.querySelectorAll(".account-balance-amount").forEach((el) => {
      const target = Number.parseFloat(el.dataset.balance);
...
document
  ...
  .addEventListener("click", async function () {
    ...
    const accountType = Number.parseInt(
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      transactions.forEach((tx) => {
        const isInternalTransfer = tx.description
          ?.toLowerCase()
          .includes("internal transfer");
      ...
      plugins: [
        {
          ...
          beforeDraw(chart) {
            ...
            const net = hasData
              ...
              : 0;
            const text = `${net >= 0 ? "+" : ""}$${Math.abs(net).toLocaleString(
              "en-CA",
              {
