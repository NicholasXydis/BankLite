...
async function loadTransactions(accountId, page, type = null) {
    ...
    <span class="transaction-date">${txDate.toLocaleString(lang === "fr" ? "fr-CA" : "en-CA", {
                hour: "numeric",
                minute: "2-digit"
            })}</span>
    </div>
    <span class="transaction-amount">${transaction.type === "Deposit" || isIncoming ? "+" : "-"}$${transaction.amount.toLocaleString("en-CA", {
    ...
    .addEventListener("click", async function () {
        ...
        try {
            ...
            result.items.forEach((tx) => {
                ...
                const amount = `${tx.type === "Deposit" ? "+" : "-"}$${tx.amount.toLocaleString("en-CA", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                })}`;
                const type =
